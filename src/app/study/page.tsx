...
const lessonBlocksMap: Record<Category, LessonBlock[]> = {
  strategy: [
    ...
    {
      title: "情報システム戦略と調達の考え方",
      summary:
        "ITパスポートでは『何を導入するか』だけでなく、『なぜ導入するか』『どう外部調達するか』も問われます。戦略と調達は経営判断の一部として整理すると覚えやすいです。",
      bullets: [
        "情報システム戦略は経営戦略を実現するためのIT活用方針で、現場の便利さだけでなく経営目標との整合が重要です。",
        "RFPは発注側が要件や期待成果を整理して提案を依頼する文書で、見積書や契約書とは役割が異なります。",
        "内製・外注・クラウド利用を比較するときは、コストだけでなくスピード、柔軟性、セキュリティ責任分界も考慮します。",
      ],
      examAngles: [
        "経営戦略と情報システム戦略の関係を問う設問",
        "RFI・RFP・SLA・契約書の役割差を見分ける設問",
        "内製・外部委託・クラウド利用の選択理由を問う設問",
      ],
      commonTraps: [
        "システム戦略を単なるハード更新計画だと考える。",
        "RFPを受注側が提出する提案書だと思い込む。",
        "外注すれば責任まで完全に移ると誤解する。",
      ],
      example: "売上拡大のためにECを強化したい企業が、要件をRFPにまとめてベンダへ提案依頼する流れは、経営戦略と情報システム戦略がつながった典型例です。",
      quickCheck: [
        "RFPが『発注側の依頼文書』だと説明できるか。",
        "IT導入目的を経営目標に結びつけて言えるか。",
      ],
      memoryHook: "戦略は『何を達成したいか』、調達は『どう実現するか』で切る。",
    },
    {
      title: "データ活用・AI・業務改革の見分け方",
      summary:
        "AI、IoT、ビッグデータ、DXは関連語として出やすいですが、技術そのものと経営上の使い方を区別できると応用問題に強くなります。",
      bullets: [
        "ビッグデータは大量・多様・高速なデータの活用、AIはその分析や予測に使われる技術、DXはそれらを使った事業変革です。",
        "IoTはデータ収集の入り口になりやすく、クラウドやAIは蓄積・分析・活用の基盤として組み合わされます。",
        "『技術導入』で終わるか、『業務や顧客体験が変わるか』まで踏み込むかでDXとの違いを見分けましょう。",
      ],
      examAngles: [
        "AI・IoT・ビッグデータ・DXの役割差を問う設問",
        "データ収集→分析→価値創出の流れを選ばせる設問",
        "PoCと本格導入、デジタル化と変革の違いを問う設問",
      ],
      commonTraps: [
        "AIを『自動化全般』として広く捉えすぎる。",
        "IoTを通信技術だけの話として終わらせる。",
        "データをためるだけでDXだと判断してしまう。",
      ],
      example: "工場のセンサからIoTで集めた稼働データをクラウドへ蓄積し、AIで故障予兆を分析して保守業務を変えるなら、技術活用から業務変革までつながったDXの例と考えられます。",
      quickCheck: [
        "IoT・AI・DXを1つの事例で役割分担できるか。",
        "単なるデータ蓄積と業務変革を区別できるか。",
      ],
      memoryHook: "集めるのがIoT、読むのがAI、変えるのがDX。",
    },
    {
  ...
  management: [
    ...
    {
      title: "テスト設計と品質保証の考え方",
      summary:
        "本番で問われやすいのは工程名の暗記だけではなく、『どの品質をどの段階で確かめるか』です。品質保証の流れを意識するとテスト体系が整理できます。",
      bullets: [
        "単体テストは部品の正しさ、結合テストは接続、システムテストは全体要件、受入テストは利用者要求の確認と整理します。",
        "機能要件だけでなく、性能・可用性・保守性など非機能要件もシステムテストで問われやすいです。",
        "品質は最後にテストだけで作るのではなく、レビューや標準化で上流から作り込む考え方が重要です。",
      ],
      examAngles: [
        "テスト工程と確認対象を対応させる設問",
        "機能要件と非機能要件の違いを問う設問",
        "品質保証活動としてレビューや標準化を選ばせる設問",
      ],
      commonTraps: [
        "受入テストを結合テストと混同する。",
        "性能確認を単体テストの役割だと考える。",
      ...
      example: "画面とDBの接続確認は結合テスト、業務シナリオどおりに全体が動くかはシステムテスト、利用部門が運用視点で確認するのは受入テストです。",
      quickCheck: [
        "受入テストの主体が利用者側だと言えるか。",
        "非機能要件の例を2つ挙げられるか。",
      ],
      memoryHook: "部品→接続→全体→利用者、の順で品質を確かめる。",
    },
    {
      title: "調達・契約・外部委託の論点整理",
      summary:
        "システム開発や運用では、ベンダとの関係や契約上の役割分担も頻出です。現場作業と契約責任を分けて理解するとひっかけに強くなります。",
      bullets: [
        "請負契約は成果物完成責任、準委任契約は業務遂行責任が中心で、責任の持ち方が異なります。",
        "SLAはサービス品質の約束、RFPは提案依頼、見積書は費用提示と、文書ごとの目的を区別します。",
        "外部委託しても情報セキュリティや業務責任が発注者から完全に消えるわけではありません。",
      ],
      examAngles: [
        "契約類型ごとの責任範囲を問う設問",
        "RFP・見積書・SLA・運用手順書の違いを問う設問",
        "委託先管理や情報セキュリティ責任を問う設問",
      ],
      commonTraps: [
        "請負と準委任をどちらも『外注』として一括りにする。",
        "SLAを金額契約書と取り違える。",
        "委託先事故の責任が自社に残ることを見落とす。",
      ],
      example: "運用監視を外部へ委託していても、顧客への説明責任や委託先の管理責任は発注側に残る、という視点は試験でも実務でも重要です。",
      quickCheck: [
        "請負と準委任の違いを一文で説明できるか。",
        "SLAとRFPの目的を混同せず言えるか。",
      ],
      memoryHook: "仕事を頼んでも、責任分界は消えない。",
    },
    {
  ...
  technology: [
    ...
    {
      title: "アルゴリズム・論理演算・データ表現",
      summary:
        "テクノロジ系の基礎点を安定させるには、計算手順だけでなく、論理的に処理を追える力も重要です。アルゴリズムとデータ表現は土台分野として押さえたい領域です。",
      bullets: [
        "AND・OR・NOTは真理値表で理解すると、条件分岐や検索条件の問題にも応用できます。",
        "文字コード、2進数、16進数、補数表現などは『情報をどう表すか』のテーマとしてまとめて整理すると覚えやすいです。",
        "流れ図や擬似言語の問題では、処理の繰り返し回数と条件判定の順番を丁寧に追うことが得点のコツです。",
      ],
      examAngles: [
        "論理演算の結果を問う設問",
        "進数変換や文字コードの基礎を問う設問",
      ...
      commonTraps: [
        "ORを『両方1のときだけ1』と誤解する。",
        "16進数と10進数の桁の意味を混同する。",
        "ループ回数を1回多く、または少なく数えてしまう。",
      ],
      example: "『AかつB』ならAND、『AまたはB』ならORです。擬似言語では、まず初期値を置き、条件判定→処理→更新の順でメモを取りながら追うと安定します。",
      quickCheck: [
        "AND/OR/NOTを真理値表で書けるか。",
        "2進数と16進数の対応を一部でも言えるか。",
      ],
      memoryHook: "論理は表、進数は桁、アルゴリズムは順番で追う。",
    },
    {
      title: "セキュアなWebとネットワーク運用",
      summary:
        "セキュリティ問題は攻撃名の暗記で終わらせず、『どこを守る仕組みか』で整理すると応用が利きます。Webとネットワーク運用の基本をつなげて理解しましょう。",
      bullets: [
        "ファイアウォール、WAF、IDS/IPSは守る場所と役割が異なります。ネットワーク境界か、Webアプリ層か、検知中心かで区別します。",
        "TLS/HTTPSは通信経路保護、電子証明書は相手確認、VPNは経路の仮想専用化と、用途がそれぞれ異なります。",
        "ログ管理や時刻同期、パッチ適用は地味ですが、攻撃発見や証跡確保のために本番で問われやすい実務論点です。",
      ],
      examAngles: [
        "WAF・ファイアウォール・IDS/IPSの違いを問う設問",
        "通信の保護と本人確認の違いを問う設問",
        "ログ・証明書・時刻同期の運用目的を問う設問",
      ],
      commonTraps: [
        "HTTPSだけでWebアプリの脆弱性対策まで完了すると考える。",
        "WAFとファイアウォールを同一視する。",
        "ログの時刻ずれが監査で問題になることを見落とす。",
      ],
      example: "SQLインジェクション対策にWAFや入力値検証が効き、通信盗聴対策にはHTTPS、ログ突合せにはNTPでの時刻同期が重要、というように目的で切り分けます。",
      quickCheck: [
        "WAFとファイアウォールの違いを言えるか。",
        "NTPがなぜセキュリティ運用で重要か説明できるか。",
      ],
      memoryHook: "守る場所・守る目的・証跡の残し方を分けて考える。",
    },
    {
